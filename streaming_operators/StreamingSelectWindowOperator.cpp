...
bool StreamingSelectWindowOperator::open(const QuerySpec *plan_specs) {
  // Error if the operator has not been closed or the spec is not for select.
  if (select_window_spec_.get() != nullptr ||
      select_window_spec_->getQuerySpecType() != kSelectWindowSpec) {
    return false;
  }

  select_window_spec_.reset(static_cast<const SelectWindowSpec*>(plan_specs));
  return true;
}

bool StreamingSelectWindowOperator::next(
...
bool StreamingSelectWindowOperator::close() {
  // Error if the operator has not been opened.
  if (select_window_spec_.get() == nullptr) {
    return false;
  }

  select_window_spec_.release();
  return true;
}
